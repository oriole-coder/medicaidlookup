...
const addressInfos = await page.$$(".col.col-1-4.provider-phone-address > .provider-address");

const planInfos = await page.$$(".col.col-1-4.benefit-plan-wrapper");
...
for (const addressInfo of addressInfos){
    ...
    try{
        providerAddress = await page.evaluate((el) => el.querySelector("p.listing-item").textContent, addressInfo);
    } catch(error) {}
...
for (const planInfo of planInfos){
    ...
    try{
        plansAvailable = await page.evaluate((el) => el.querySelector(".provider-plans").textContent, planInfo);
    } catch(error) {}
